...
public class AudioEncoder {
    ...
    private ExecutorService encodingService = null;//Executors.newSingleThreadExecutor(); // re-use encodingService
    private String outputFile;
    private int samplingRate;
    private double audioSize = 0;
    //AudioSoftwarePoller audioSoftwarePoller;
    WebRtcAudioRecord audioSoftwarePoller;
    private int sizeUpdateCounter = 0;

    public AudioEncoder(Context c, int samplingRate) {
        this.c = c;
        this.samplingRate = samplingRate;
        //prepare();
    }

    public void prepareEncoder(String outputFile){
        if (encodingService==null||encodingService.isShutdown()||encodingService.isTerminated()) {
            encodingService = Executors.newSingleThreadExecutor(); // re-use encodingService
        }
        this.outputFile = outputFile;
        if (outputFile.indexOf("_audio")==-1){
            this.outputFile = outputFile.replaceAll(".mp4", "_audio.mp4");
        }
        try {
    ...
    public void setAudioSoftwarePoller(WebRtcAudioRecord audioSoftwarePoller) {
        this.audioSoftwarePoller = audioSoftwarePoller;
    }
    ...
    private void prepare() {
        ...
        sizeUpdateCounter=0;
        frameCount = 0;
        //TeamApp.audioBytes =0;
                eosReceived = false;
        eosSentToAudioEncoder = false;
        stopReceived = false;
        //File f = FileUtils.createTempFileInRootAppStorage(c, "test_" + new Date().getTime() + ".mp4");
        ...
        audioFormat.setInteger(MediaFormat.KEY_SAMPLE_RATE, samplingRate);
    ...
    public void stop() {
        ...
        //if (!encodingService.isShutdown()) {
            //encodingService.shutdown();//stop receiving anymore requests
        ...
        //}
        Log.d(TAG, "Audio Size " + audioSize);
    }
    ...
    public void offerAudioEncoder(byte[] input, long presentationTimeStampNs) {
        if (!encodingService.isShutdown()) {
            ...
            //_offerAudioEncoder(input, presentationTimeStampNs);
