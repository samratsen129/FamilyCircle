...
/**
 * Created by samratsen on 4/29/15.
 */
public class AudioPollerEncoder {
    private static final String TAG = "AudioPollerEncoder";
    ...
    public AudioPollerEncoder (Context c) {
        this.c = c;
        //prepare();
    }
    ...
    private class EncoderTask implements Runnable {
        ...
        private AudioPollerEncoder encoder;
        private EncoderTaskType type;
        private byte[] audio_data;

        public EncoderTask(AudioPollerEncoder encoder, EncoderTaskType type) {
            setEncoder(encoder);
            this.type = type;
            switch (type) {
                /*
        ...
        public EncoderTask(AudioPollerEncoder encoder, byte[] audio_data, long pts) {
            setEncoder(encoder);
            setEncodeFrameParams(audio_data, pts);
        }

        public EncoderTask(AudioPollerEncoder encoder) {
            setEncoder(encoder);
            setFinalizeEncoderParams();
        }

        private void setEncoder(AudioPollerEncoder encoder) {
