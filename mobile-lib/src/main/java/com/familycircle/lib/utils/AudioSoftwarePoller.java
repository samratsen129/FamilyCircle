...
public class AudioSoftwarePoller {
    ...
    WeakReference<AudioPollerEncoder> audioEncoder;

    public AudioSoftwarePoller() {
    }


    public void setAudioEncoder(AudioPollerEncoder avcEncoder) {
        this.audioEncoder = new WeakReference<AudioPollerEncoder>(avcEncoder);
    }
    ...
    public class RecorderTask implements Runnable {
        ...
        public void run() {
            ...
            while (is_recording) {
                ...
                if (VERBOSE)
                    Log.i("AudioSoftwarePoller", String.valueOf(buffer_write_index) + " - " + String.valueOf(buffer_write_index + samples_per_frame - 1));
                ...
                if(audioEncoder != null){
                    audioEncoder.get().offerAudioEncoder(this_buffer, audioPresentationTimeNs);
                }
            }
